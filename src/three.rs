...
impl TronMatrix {
    ...
    fn add_base_segment(&mut self, a: Point, b: Point) {
        let lt_x = cmp::min(a.x, b.x);
        let gt_x = cmp::max(a.x, b.x);
        for x in lt_x..=gt_x {
            let lt_y = cmp::min(a.y, b.y);
            let gt_y = cmp::max(a.y, b.y);
            for y in lt_y..=gt_y {
                self.matrix.insert(Point { x: x, y: y });
                println!("PURT IN {} {}", x, y)
            }
        }
    }

    fn check_segment(&mut self, a: Point, b: Point) {
        let lt_x = cmp::min(a.x, b.x);
        let gt_x = cmp::max(a.x, b.x);
        for x in lt_x..=gt_x {
            let lt_y = cmp::min(a.y, b.y);
            ...
            for y in lt_y..=gt_y {
                ...
                if self.matrix.contains(&Point { x: x, y: y }) {
                    println!("CHECKING {} {}", x, y);
                    self.intersections.push(Point { x: x, y: y })
                }
            }
        }
    }
...
fn wire_up(one_in: &str, two_in: &str) -> i32 {
    let mut last = Point { x: 0, y: 0 };
    let mut matrix = TronMatrix::new();
    for cmd in one_in.split(",") {
        ...
        matrix.add_base_segment(last, next);
        last = next;
    }
    ...
    for cmd in two_in.split(",") {
        ...
        matrix.check_segment(last, next);
        last = next;
    }

    print!("INTER, {:?}", matrix.intersections);
    let closest_point = matrix.intersections.iter().fold(
        Point {
            x: 999999,
            y: 999999,
        },
    ...
    return closest_point.manhatten();
}

pub fn three_a() -> i32 {
    wire_up(WIRE_ONE, WIRE_TWO)
}
...
mod tests {
    ...
    fn start() {
        ...
        matrix.add_base_segment(Point { x: 0, y: 22 }, Point { x: 10, y: 22 });

        matrix.check_segment(Point { x: 3, y: 10 }, Point { x: 3, y: 30 });
    ...
    #[test]
    fn the_basics() {
        assert_eq!(wire_up("R8,U5,L5,D3", "U7,R6,D4,L4"), 6);
        assert_eq!(
            wire_up(
